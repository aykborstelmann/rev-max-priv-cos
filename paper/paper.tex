...
\begin{section}{Single Buyer Mechanism Design}
 ...
 For the first part, in common single parameter mechanisms \citet{myerson} found a clear characterization of a truthful mechanism (\Cref{lemma:myersons-lemma}).
 ...
 by generalizing a truthful single-parameter mechanism to a one that takes participation cost into account (\Cref{lemma:thruthful-mechanism}).
 ...
 In the single parameter setting, again \citet{myerson} provided such as well (\Cref{lemma:myerson-revenue}).
 ...
 For that we first formulize the mechanism described in \Cref{lemma:thruthful-mechanism}.
 ...
 \begin{figure}[htp!]
     \centering
     \begin{tikzpicture}[
             node/.style={circle, draw=black, fill=gray!20, scale=.3},
             axis/.style={->, thick},
