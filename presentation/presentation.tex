...
\begin{frame}{Recursive Step - Lower Value}
  ...
  \only<1>{
    \begin{align*}
      u(i\epsilon)     & = \int_0^{i \epsilon} x(t)dt - p_0 = \underbrace{\int_0^{(i-1) \epsilon} x(t)dt - p_0}_{u((i-1)\epsilon)} + \int_{(i-1)\epsilon}^{i\epsilon} \underbrace{x(t)}_{\text{const}}dt \\
                       & = u((i-1)\epsilon) + \epsilon x(i\epsilon) = u((i-1)\epsilon) + j \epsilon^2                                                                                                    \\
                       & \Leftrightarrow                                                                                                                                                                 \\
  ...
  \only<2>{
    \begin{itemize}
      \item $u(v') = (k-j)\epsilon^2$
      \item $x(v') \leq j \epsilon$ (lower values should not be allocated more)
    \end{itemize}
    \begin{align*}
        & \max_x\mathbf{E}\left[p(v) \mid v \leq (i - 1) \epsilon, x(i\epsilon) = j\epsilon, u(i\epsilon) = k\epsilon^2 \right]                    \\
      = & \max_x\mathbf{E}\left[p(v) \mid v \leq (i - 1) \epsilon, x((i-1)\epsilon) \leq j\epsilon, u((i-1)\epsilon) = (k-j)\epsilon^2 \right]     \\
      = & \max_{j' \leq j}\mathbf{E}\left[p(v) \mid v \leq (i - 1) \epsilon, x((i-1)\epsilon) = j'\epsilon, u(i\epsilon) = (k-j)\epsilon^2 \right] \\
