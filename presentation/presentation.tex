...
\begin{frame}{FPTAS Algorithm - Dynamic Programming}
  ...
  \begin{itemize}
    ...
    \item Round allocation function down to the nearest multiple of $\epsilon$
  \end{itemize}
...
\begin{frame}{Dynamic Programming - Recursive Step - Lower Value}
  \begin{align*}
    \alert{\max_{\mathcal{M}' : x(i\epsilon) = j\epsilon, u(i\epsilon) = k\epsilon^2} \mathbf{E}\left[p_c(v) \mid v \leq (i - 1) \epsilon\right]} = \max_x R(i-1, ?, \alt<1>{\alert<1>{?}}{k-j})
  \end{align*}

  \begin{itemize}
    \item $u((i-1)\epsilon) = \alt<1>{\alert<1>{?}}{(k-j)\epsilon^2}$
  ...
  \only<2>{
    \begin{align*}
      \Rightarrow & \max_{\mathcal{M}' : x(i\epsilon) = j\epsilon, u(i\epsilon) = k\epsilon^2}\mathbf{E}\left[p_c(v) \mid v \leq (i - 1) \epsilon \right] = \max_{j' \leq j} R(i-1, j', k-j) \\
    \end{align*}
  }
\end{frame}
...
\begin{frame}{Dynamic Programming - Recursive Step}
  ...
  \begin{align*}
     & R(i,j,k)                                                                                                                                           \\
     & = \max_{j' \leq j} R(i-1, j', k-j) + \alert<2>{\mathbf{Pr}[v = i\epsilon \mid v \leq i \epsilon] \mathbf{E}\left[p_c(v) \mid v = i\epsilon\right]}
  \end{align*}
